  ...
  return (target: any, handlerKey: string, descriptor: PropertyDescriptor) => {
    ...
    descriptor.value = async (req: HttpRequest, ctx: InvocationContext) => {
      try {
        ...
        const graphToken =
          req.headers.get("x-graph-token") || req.headers.get("X-Graph-Token");
        if (!graphToken) {
          console.error("Missing graph token header");
          return {
            status: STATUS_CODES.BAD_REQUEST,
            jsonBody: {
              message: "No se proporcionó token de Graph en el header",
            },
          };
        }
        ...
        if (!response.ok) {
          ...
          if (response.status === 401) {
            ...
            return {
              ...
              jsonBody: {
                message: "Sin permisos para acceder a Microsoft Graph",
              },
            };
          } else {
            return {
              status: STATUS_CODES.BAD_REQUEST,
              jsonBody: {
                message: "Error al obtener información del usuario de Graph",
              },
            };
          }
        }
        ...
        try {
          ...
          return {
            status: STATUS_CODES.INTERNAL_SERVER_ERROR,
            jsonBody: {
              message: "Error al procesar respuesta de Microsoft Graph",
            },
          };
        }
        ...
        if (!levels.includes(user.role)) {
          console.log(
            `Access denied. User role '${user.role}' not in allowed levels:`,
            levels,
          );
          return {
            status: STATUS_CODES.FORBIDDEN,
